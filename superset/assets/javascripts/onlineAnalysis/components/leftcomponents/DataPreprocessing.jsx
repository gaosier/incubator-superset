...
class DataPreprocessing extends Component {
    ...
    onEnterModal(){
        if(this.props.leftmenu.form_data.description_img.name===null){
            return(
            <div>没有上传图片或上传图片不成功,请重新上传</div>)
        }else{
            const url_img = "/static/uploads/img/"+this.props.leftmenu.form_data.description_img.name;
            return(
                <div>
                <img src={url_img}/>
                    </div>
        ...
        const props = {
            ...
            onChange(info) {
                if (info.file.status !== 'uploading') {
                    set_description_img(info.file.response.url,info.file.response.filename);
                }
