...
class DatabaseView(SupersetModelView, DeleteMixin):  # noqa
    ...
    add_columns = [
        ...
        'allow_ctas', 'allow_dml', 'force_ctas_schema', 'is_hybrid']
...
class Superset(BaseSupersetView):
    ...
    def explore_json(self, datasource_type, datasource_id):
        ...
        #将查询结果改为中文
        # print(viz_obj.datasource.columns, 7777777777777777777)
        # dic={m.column_name : (m.verbose_name or m.column_name) for m in viz_obj.datasource.columns}
    ...
    def explore(self, datasource_type, datasource_id):
        ...
        # print(action, 3333333333333333333333333333333333333)
    ...
    def csv(self, client_id):
        ...
        if blob:
            ...
            csv = df.to_csv(index=False, encoding='utf-8')
        response = Response(csv.encode('GBK'), mimetype='text/csv')
        response.headers['Content-Disposition'] = (
            'attachment; filename={}.csv'.format(query.name).encode('utf-8'))
...
class FilterRelationManyToManyEqual(FilterRelation):
    ...
    def apply(self, query, value):
        print(self.model, self.column_name)
        ...
        for v in value:
            ...
            query.filter(field.contains(rel_obj))
            print(888,type(query))
...
class MElementView(SupersetModelView):
    datamodel = SQLAInterface(models.MElement)
    list_title = '点击列表'
    show_title = '点击详情'
    add_title = '添加点击信息'
    edit_title = '编辑点击信息'
